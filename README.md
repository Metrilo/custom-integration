...
Once you've [signed up for a Metrilo account](https://app.metrilo.com/signup), navigate to the `Settings -> Installation` page in your [Metrilo project](https://app.metrilo.com) and keep your `API Token` and `API Secret` handy - you'll need them for the rest of this tutorial:
...
Let's create a category, a product and a customer.
We'll assume that:
...
:information_source: Metrilo API uses a custom `X-Digest` header, which is an HMAC-SHA256 digest generated by using the request body string as the message and your Metrilo API secret as the key. You have to generate a separate digest for every resource call you make.
...
If you have integrated with the previous version of our API, the guide has been moved [here](api_v1.md).
...
:information_source: Each backend call except `/customer` **must** be sent with an additional custom header called `X-Digest`, which is an HMAC-SHA256 digest generated by using the request body string as the message and your Metrilo API secret as the key. [This list](https://github.com/danharper/hmac-examples) provides great examples in different programming languages, which can help you with the implementation.
...
:warning: If your form is not part of the DOM when the page is loaded (e.g. it appears after 30 seconds or whenever a user scrolls a certain amount), you would need to call that script *after* the form appears on the site so that the form is picked up.
